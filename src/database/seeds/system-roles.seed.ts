...
export async function seedRoles(dataSource: DataSource) {
  ...
  const rolesData = [
    ...
    },
    {
      ...
      permissions: [
        'user:*', 'role:*', 'acl:*', 'expense:*', 'payroll:*', 
        'invoice:*', 'report:*', 'workflow:*', 'notification:*'
    ...
    },
    {
      ...
      permissions: ['audit:read', 'report:read', 'expense:read', 'payroll:read', 'invoice:read'],
    },
    {
      name: 'Employee',
    ...
    },
    {
      ...
      permissions: ['expense:read', 'expense:approve', 'payroll:read', 'report:read'],
    },
    {
      name: 'Billing Manager',
      description: 'Access to invoices and billing',
      permissions: ['invoice:read', 'invoice:write', 'invoice:approve', 'report:read', 'report:export'],
    }
  ];
  ...
  for (const roleDef of rolesData) {
    let existingRole = await roleRepo.findOne({ where: { name: roleDef.name, is_system: true, tenant_id: IsNull() } });
    ...
    for (const p of roleDef.permissions) {
      if (p.endsWith(':*')) {
        ...
        const matching = allPerms.filter(ap => ap.action.startsWith(`${resource}:`));
    ...
    for (const perm of permissionsToAssign) {
      const existingRp = await rolePermissionRepo.findOne({ where: { role_id: existingRole.id, permission_id: perm.id } });
      if (!existingRp) {
